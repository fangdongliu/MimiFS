...
REGISTER_HANDLER(HandleImport, "import", "将外部文件(夹)导入系统", "import \"winFilepath\" \"filepath\"\nwinFilepath:外部路径\nfilepath:MiniFS文件路径")
...
void HandleImport::onHandleCommand(Lexer&param) {
...

	{
		...
		cout << "导入文件夹数：" << cnt1 << endl;
		cout << "导入文件数：" << cnt2 << endl;

		return;
	}

importError:
	cout << "import \"filepath\" \"winFilepath\" \"filename\"";
